...
class ExperimentTree(ttk.Frame):
    def __init__(self,parent):
        ...
        self.filtered_tree.bind('<Double-Button-1>', lambda event: inspect_wrapper(event, self.filtered_tree, parent.manager))
        ...
        self.filter_btn = tk.Button(self.filtering_actions_frame, text = 'Filter', command = lambda: filter_experiments(self.filtered_tree, parent.manager, self.current_filter_var))
        self.filter_btn.grid(row = 0, column = 0, padx = 5, pady = 5, sticky = 'w')
        self.reset_filter_btn = tk.Button(self.filtering_actions_frame, text = 'Reset', command = lambda: reset(self.filtered_tree, parent.manager))
